with "provider_retrievability_weekly" as (select date_trunc('week', "provider_retrievability_daily"."date")                                               as "week",
                                                 ...
                                                 sum(coalesce("provider_url_finder_retrievability_daily"."success_rate", 0)::float) / sum("total"::float) as "success_rate_url_finder"
                                          from "provider_retrievability_daily"
                                                   left join "provider_url_finder_retrievability_daily"
                                                             on "provider_retrievability_daily"."provider" = "provider_url_finder_retrievability_daily"."provider"
