with provider_retrievability_weekly as (
    select distinct on (puf.provider, date_trunc('week', puf.date))
        date_trunc('week', puf.date)                  as week,
        puf.provider                                  as provider,
        puf.success_rate                              as success_rate_url_finder
    from provider_url_finder_retrievability_daily puf
    order by
        puf.provider,
        date_trunc('week', puf.date),
        puf.date desc
)
select
    coalesce(prw.week, date_trunc('week', cpwa.week)) as week,
...
group by
    coalesce(prw.week, date_trunc('week', cpwa.week)),
    cpwa.provider;