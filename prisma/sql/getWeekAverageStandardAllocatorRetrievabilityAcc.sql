...
with "active_allocators" as (
    select "allocator_id", "registry_info", 6 as "editionId"
    from "allocator_registry"
    where "rejected" = false

    UNION ALL

    select "allocator_id", "registry_info", 5 as "editionId"
...
"open_data_pathway_allocators" as (
    ...
    where "active_in_edition"."editionId" = 5
      and lower("allocator_client_bookkeeping"."bookkeeping_info"::jsonb->'Project'->>
...
where (
    $1 = false
        or "allocator" in (select "allocator_id" from "open_data_pathway_allocators")
    )
